...
import { resolveChildExtensionPaths, resolvePiInvocation } from "./invocation.js";
...
async function runSingleAgent(
	...
	for (const extensionPath of resolveChildExtensionPaths(defaultCwd)) {
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			const invocation = resolvePiInvocation(args, { cwd: defaultCwd });
		...
		renderResult(result, { expanded }, theme, _context) {
			...
			if (details.mode === "single" && details.results.length === 1) {
				...
				if (expanded) {
					...
					const usageStr = formatUsageStats(current.usage, current.model);
					if (usageStr) {
						container.addChild(new Spacer(1));
			...
			const aggregateUsage = (results: SingleResult[]) => {
				...
				for (const resultEntry of results) {
					total.input += resultEntry.usage.input;
					total.output += resultEntry.usage.output;
					total.cacheRead += resultEntry.usage.cacheRead;
					total.cacheWrite += resultEntry.usage.cacheWrite;
					total.cost += resultEntry.usage.cost;
					total.turns += resultEntry.usage.turns;
				}
				return total;
			};

			if (details.mode === "chain") {
				const successCount = details.results.filter((resultEntry) => resultEntry.exitCode === 0).length;
				...
				if (expanded) {
					...
					for (const resultEntry of details.results) {
						const entryIcon = resultEntry.exitCode === 0 ? theme.fg("success", "✓") : theme.fg("error", "✗");
						const displayItems = getDisplayItems(resultEntry.messages);
						const finalOutput = getFinalOutput(resultEntry.messages);

						container.addChild(new Spacer(1));
						container.addChild(
							new Text(
								`${theme.fg("muted", `─── Step ${resultEntry.step}: `) + theme.fg("accent", resultEntry.agent)} ${entryIcon}`,
								0,
								0
							)
						);
						container.addChild(new Text(theme.fg("muted", "Task: ") + theme.fg("dim", resultEntry.task), 0, 0));
						...
						const stepUsage = formatUsageStats(resultEntry.usage, resultEntry.model);
			...
			if (details.mode === "parallel") {
				const running = details.results.filter((resultEntry) => resultEntry.exitCode === -1).length;
				const successCount = details.results.filter((resultEntry) => resultEntry.exitCode === 0).length;
				const failCount = details.results.filter((resultEntry) => resultEntry.exitCode > 0).length;
				...
				if (expanded && !isRunning) {
					...
					for (const resultEntry of details.results) {
						...
						container.addChild(new Spacer(1));
						container.addChild(
							new Text(`${theme.fg("muted", "─── ") + theme.fg("accent", resultEntry.agent)} ${entryIcon}`, 0, 0)
						);
						container.addChild(new Text(theme.fg("muted", "Task: ") + theme.fg("dim", resultEntry.task), 0, 0));
						...
						const taskUsage = formatUsageStats(resultEntry.usage, resultEntry.model);
				...
				for (const resultEntry of details.results) {
					const entryIcon =
						resultEntry.exitCode === -1
							? theme.fg("warning", "⏳")
							: resultEntry.exitCode === 0
								? theme.fg("success", "✓")
								: theme.fg("error", "✗");
					const displayItems = getDisplayItems(resultEntry.messages);
					text += `\n\n${theme.fg("muted", "─── ")}${theme.fg("accent", resultEntry.agent)} ${entryIcon}`;
					if (displayItems.length === 0) {
						text += `\n${theme.fg("muted", resultEntry.exitCode === -1 ? "(running...)" : "(no output)")}`;
					} else {
						text += `\n${renderDisplayItems(displayItems, 5)}`;
					}
				}
